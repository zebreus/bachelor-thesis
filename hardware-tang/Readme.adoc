...
The synthesized interface also looks sane, so we can attempt the next step and try to get the module loaded to the FPGA. 
...
Here something interesting happens, as we can see that the module writes 4 values to the memory in just two cycles.
