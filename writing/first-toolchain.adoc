...
With FPGAs, we'll transcend the limitations of traditional CPUs, unlocking faster and more efficient calculations. Our journey will be a perilous one, as the language of FPGAs and the common languages of our world are vastly different. It would need a mighty sorcerer to bridge that gap. Maybe even multiple ones. The path ahead is unclear, but we shall not be deterred.
...
A few weeks ago, I decided that I am interested in FPGAs and how Rust could be used to program them. In this post I will document my first explorations into HLS and Rust. We will learn what HLS is, how it works and how we can use it to synthesize Rust code.

== What is the difference between CPUs and FPGAs

Normal CPUs process their instructions one by one. They are purposebuild machines that are carefully designed for processing lots of instructions in sequence. Tradional programminglanguages reflect this design for the most part. They are designed to make it easy for humans to write programs that can be executed one instruction at a time, from top to bottom. There are approached to paralelism, but they are mostly limited to having multiple threads of execution that run from top to bottom simulataneously, or to have some instructions that perform the same instruction on a fixed amount of data elements at the same time.

FPGAs on the other hand are not designed to process one instructions at a time. They are not even designed to process instructions at all. FPGA stands for field programmable gate array. As the name implies, it is basically just a lot of logic gates that can be connected via a program. So while a CPU is a circuit designed to do one thing, a FPGA is a circuit designed to emulate other circuits. So you could for example define the logic gates that make up your CPU and program your FPGA with that, so it will behave exactly like a CPU and can process instructions. But you could also define the logic to perform a complex calculation that would take a CPU hundreds of cycles, deploy it to a FPGA and you have a piece of hardware that can perform that calculation in an instant.

The downsides of using FPGAs is that they are expensive and harder to program than a CPU. They are expensive because they need to be a lot bigger than circuits that are directly designed to to something. While a logic gate in a real circuit are just a few transistors, in an FPGA you need more <TODO: do some research?>. Also the connections between them need to be configurable, TODO: research this stuff. 

[quote,"Bacon, David F., Rodric Rabbah, and Sunil Shukla.",'https://dl.acm.org/doi/fullHtml/10.1145/2436256.2436271["FPGA programming for the masses." Communications of the ACM 56.4 (2013)]']
____
...
Also verification is a bit part of hardware design. TODO: I will add more on this later. I assume this is, because in traditional hardware design the goal is to produce actual circuits, where each revision is extremly expensive (at least multiple hundred thousand dollars), so you want to make sure you get it right the first time. In software, you can just change the code, recompile it, and ship an update. Also as everything happens in parralel, it is possible to get timing related problems.

// A FPGA is programmed not in a programming language, but in a hardware definition language. While hardware definition languages look somewhat similar to programming languages, they They are designed to be reprogrammable and to be able to process multiple things at once. They achieve this by beeing one of the simplest possible circuit

// Our languages merely communicate a list of instructions that can be understood one at a time. There may be some of us that can pretend to understand multiple commands at once, but I say those are liers. not more than a bunch of children covering themselves in an oversized trenchcoat trying to pass of as one adult. There may be 4 or 8 or even 64 of them in there but each one is still only capable of doing one thing at a time. To the FPGAs on the other hand even the concepts of instructions or order is foreign. In their tongue the primitives are the connection and the lookup table (LUT). They not so much understand a instruction at a time, but adjust their self to mirror the words. They can, on under special circumstances, understand the language of the CPU, but they do this only by adjusting their structure to replicate a CPU. As you maybe can imagine they are not very good at this. To replicate a 

== What even is a HLS?

Raise the level of abstraction of the design compared to HDLs.
