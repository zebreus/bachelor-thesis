...
The {cpp} implementations will be synthesized using bambu with both the GCC and clang frontend. Rust implementations will be synthesized using the toolchain described in the previous section, which is also based around bambu. All implementations will be synthesized once with optimizations for size (`-Os`) and once with optimizations for speed (`-O3`).

Different designs will be evaluated for the area of a FPGA they take up, their clock maximum frequency and the amount of clock cycles they take for a single operation. In case a design has a varying amount of clock cycles the average of all testcases will be used. The area and maximum frequency of a design will be taken from the placement report of nextpnr. nextpnr will be configured to target a Lattice ECP5 FPGA with 44k LUT and a speed grade of 6 (LFE5U-45F-6BG381C) for all tests. The amount of clock cycles will be measured using the rust_hdl simulator for Rust designs and the Verilator simulator for C++ designs. 
...
The follwing combinations of optimizations and implementations will be tested:

* minmax_rust_speed
* minmax_rust_size
* minmax_rust_idiomatic_speed
* minmax_rust_idiomatic_size
* minmax_gcc_speed
* minmax_gcc_size
* minmax_clang_speed
* minmax_clang_size
