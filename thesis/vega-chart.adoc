...
[subs="attributes"]
++++
    <div class="diagram imageblock" style="display: flex; justify-content: center; break-inside: avoid;"></div>
    <script>
      if(
        !document.vegaLoaded &&
...
[vegalite.completly-oversized-content,opts=inline,{chart-id}]
....
include::{vega-lite-filename}[]
....
:chart-id:
